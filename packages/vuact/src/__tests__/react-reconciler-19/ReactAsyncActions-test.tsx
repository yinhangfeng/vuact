...
describe('ReactAsyncActions', () => {
  beforeEach(() => {
    ...
    if (textCache) {
      for (const [text, record] of textCache) {
        resolveText(text);
      }
    }
  ...
  let setStepB;
    ...
    if (!__VUACT_SKIP_TEST__) {
      // TODO vuact2 需要实现 startTransition 中触发的 setState 如果产生了 thenable 错误，则 transition 需要等待 thenable 结束
      expect(root).toMatchRenderedOutput(
        <>
          <span>Pending: true</span>
          <span>A0, B0, C0</span>
  ...
  it(
    ...
    async () => {
      ...
      assertLog(['A']);
      if (!__VUACT_SKIP_TEST__) {
        // TODO vuact2 class component setState 在 traisition 中只会保证首次不更新，如果由于其它原因导致组件更新则 pendingStates 会被执行
        expect(root).toMatchRenderedOutput(<span>A</span>);
      }
