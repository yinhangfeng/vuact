...
describe('ReactUse', () => {
  beforeEach(() => {
    ...
    waitForAll = () => new Promise((resolve) => setTimeout(resolve, 100));
    assertLog = () => {}
    waitForPaint = () => new Promise((resolve) => setTimeout(resolve, 100));
    waitFor = () => new Promise((resolve) => setTimeout(resolve, 100));
    waitForMicrotasks = () => new Promise((resolve) => setTimeout(resolve, 100));
  ...
  function getCachableAsyncText(text) {
    let requests = pendingTextRequests.get(text);
    if (!requests) {
      requests = []
      requests.promise = new Promise(resolve => {
        requests.push((text) => {
          resolve(text)
        })
      });
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'if suspended fiber is pinged in a microtask, it does not block a transition from completing', async () => {
    let fulfilled = false;
    function Async() {
      if (fulfilled) {
        return <Text text="Async" />;
  ...
  // TODO vuact3 对于 use 未缓存的 promise 如何防止死循环
  it.skipIf(__VUACT_SKIP_TEST__, "using a promise that's not cached between attempts", async () => {
    function Async() {
      const text =
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'erroring in the same component as an uncached promise does not result in an infinite loop', async () => {
    class ErrorBoundary extends React.Component {
      state = {error: null};
      static getDerivedStateFromError(error) {
        return {error};
  ...
  // TODO vuact3 lazy 支持 load 直接返回 react element，react 文档中没有这种用法?
  it.skipIf(__VUACT_SKIP_TEST__, 'interrupting while yielded should reset contexts', async () => {
    let resolve;
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'during a transition, can unwrap async operations even if nothing is cached', async () => {
    function App() {
  ...
  // @gate enableSuspendingDuringWorkLoop
  it.skipIf(__VUACT_SKIP_TEST__, "does not prevent a Suspense fallback from showing if it's a new boundary, even during a transition", async () => {
    function App() {
      return <Text text={use(getAsyncText('Async'))} />;
    }
  ...
  // @gate enableSuspendingDuringWorkLoop
  it.skipIf(__VUACT_SKIP_TEST__, 'when waiting for data to resolve, an update on a different root does not cause work to be dropped', async () => {
  ...
  it('when replaying a suspended component, reuses the hooks computed during the previous attempt (State)', async () => {
    ...
    function Kitchen() {
      ...
      _setFruit = setFruit;
      const usedFruit = use(getCachableAsyncText(fruit));
  ...
  it('when replaying a suspended component, reuses the hooks computed during the previous attempt (DebugValue+State)', async () => {
    ...
    function Lexicon() {
      useDebugValue(123);
      const avocado = use(getCachableAsyncText('aguacate'));
  ...
  // @gate enableSuspendingDuringWorkLoop
  it.skipIf(__VUACT_SKIP_TEST__, 'load multiple nested Suspense boundaries (uncached requests)', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'use() combined with render phase updates', async () => {
    function Async() {
  ...
  // @gate enableSuspendingDuringWorkLoop
  it.skipIf(__VUACT_SKIP_TEST__, 'basic async component', async () => {
    async function App() {
  ...
  // @gate enableSuspendingDuringWorkLoop
  it.skipIf(__VUACT_SKIP_TEST__, 'async child of a non-function component (e.g. a class)', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'async children are transparently unwrapped before being reconciled (top level)', async () => {
    function Child({text}) {
      useEffect(() => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'unwrap uncached promises inside forwardRef', async () => {
    const asyncInstance = {};
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'unwrap uncached promises inside memo', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'unwrap uncached promises in component that accesses legacy context', async () => {
  ...
  it('regression test: updates while component is suspended should not be mistaken for render phase updates', async () => {
    ...
    if (__VUACT_SKIP_TEST__) {
      await act(() => resolveTextRequests('B'));
    }
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'an async component outside of a Suspense boundary crashes with an error (resolves in microtask)', async () => {
    class ErrorBoundary extends React.Component {
      state = {error: null};
      static getDerivedStateFromError(error) {
        return {error};
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'an async component outside of a Suspense boundary crashes with an error (resolves in macrotask)', async () => {
    class ErrorBoundary extends React.Component {
      state = {error: null};
      static getDerivedStateFromError(error) {
        return {error};
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'warn if async client component calls a hook (e.g. use)', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'async generator component', async () => {
    let hi, world;
    async function* App() {
  ...
  // @gate enableAsyncIterableChildren
  it.skipIf(__VUACT_SKIP_TEST__, 'async iterable children', async () => {
    let hi, world;
    const iterable = {
