...
describe('ReactHooks', () => {
  ...
  it('bails out in the render phase if all of the state is the same', () => {
    ...
    act(() => setCounter1(1));
    // TODO vuact3 useState update 时如果前后一致不会触发 render
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 1, 1'] : []);
  ...
  it('bails out in render phase if all the state is the same and props bail out with memo', () => {
    ...
    // Update that bails out.
    act(() => setCounter1(1));
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 1, 1 (light)'] : []);
    ...
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 1, 2 (dark)'] : []);
  });
  ...
  it('never bails out if context has changed', () => {
    ...
    act(() => setCounter(1));
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 1 (light)'] : []);
    expect(root).toMatchRenderedOutput('1 (light)');
  ...
  it('can bail out without calling render phase (as an optimization) if queue is known to be empty', () => {
    ...
    act(() => setCounter(1));
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 1'] : []);
    expect(root).toMatchRenderedOutput('1');
    ...
    expect(Scheduler).toHaveYielded(!__VUACT_SKIP_TEST__ ? ['Parent: 0'] : []);
    expect(root).toMatchRenderedOutput('0');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'can rebase on top of a previously skipped update', () => {
    const {useState} = React;
  ...
  it.skipIf(__VUACT_SKIP_TEST__, "throws when calling hooks inside .memo's compare function", () => {
    const {useState} = React;
    function App() {
      useState(0);
      return null;
  ...
  it.skipIf(!__VUACT_SKIP_TEST__, 'warns when calling hooks inside useReducer', () => {
  ...
  describe('hook ordering', () => {
    ...
    hooksInList.forEach((firstHelper, index) => {
      ...
      it.skipIf(__VUACT_SKIP_TEST__, `warns when fewer hooks (${hookNameA}, ${hookNameB}) are used during update than mount`, () => {
        function App(props) {
          /* eslint-disable no-unused-vars */
          if (props.update) {
            firstHelper();
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'detects a bad hook order even if the component throws', () => {
