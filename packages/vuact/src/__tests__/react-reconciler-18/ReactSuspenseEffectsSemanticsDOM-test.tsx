...
describe('ReactSuspenseEffectsSemanticsDOM', () => {
  ...
  it('should not cause a cycle when combined with a render phase update', () => {
    ...
    if (!__VUACT_SKIP_TEST__) {
      // TODO vuact3 会死循环 react 也会但不会阻塞
      act(() => {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not destroy ref cleanup twice when hidden child is removed', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not call componentWillUnmount twice when hidden child is removed', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not destroy layout effects twice when parent suspense is removed', async () => {
    function ChildA({label}) {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not destroy ref cleanup twice when parent suspense is removed', async () => {
    function ChildA({label}) {
      return (
        <span
          ref={node => {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not call componentWillUnmount twice when parent suspense is removed', async () => {
    class ChildA extends React.Component {
      componentDidMount() {
        Scheduler.unstable_yieldValue('Did mount: ' + this.props.label);
      }
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'regression: unmount hidden tree, in legacy mode', async () => {
