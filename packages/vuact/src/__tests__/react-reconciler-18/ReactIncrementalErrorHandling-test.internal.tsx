...
describe.skip('ReactIncrementalErrorHandling', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'recovers from errors asynchronously', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, "retries at a lower priority if there's additional pending work", async () => {
    function App(props) {
      if (props.isBroken) {
  ...
  // @gate www
  it.skipIf(__VUACT_SKIP_TEST__, 'does not include offscreen work when retrying after an error', () => {
    function App(props) {
      if (props.isBroken) {
        Scheduler.unstable_yieldValue('error');
        throw new Error('Oops!');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'retries one more time if an error occurs during a render that expires midway through the tree', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'catches render error in a boundary during partial deferred mounting', () => {
    class ErrorBoundary extends React.Component {
      state = {error: null};
      componentDidCatch(error) {
