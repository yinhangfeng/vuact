...
describe.skip('ReactOffscreen', () => {
  ...
  // @gate www
  it.skipIf(__VUACT_SKIP_TEST2__, 'unstable-defer-without-hiding should never toggle the visibility of its children', async () => {
    function App({mode}) {
      return (
        <>
          <Text text="Normal" />
  ...
  // @gate www
  it.skipIf(__VUACT_SKIP_TEST2__, 'does not defer in legacy mode', async () => {
    let setState;
    function Foo() {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'mounts without layout effects when hidden', async () => {
    function Child({text}) {
      useLayoutEffect(() => {
  ...
  // @gate experimental || www
  // @gate enableSuspenseLayoutEffectSemantics
  it.skipIf(__VUACT_SKIP_TEST2__, 'mounts/unmounts layout effects when visibility changes (starting hidden)', async () => {
    function Child({text}) {
      useLayoutEffect(() => {
        Scheduler.unstable_yieldValue('Mount layout');
        return () => {
  ...
  // @gate experimental || www
  // @gate enableSuspenseLayoutEffectSemantics
  it.skipIf(__VUACT_SKIP_TEST2__, 'hides children of offscreen after layout effects are destroyed', async () => {
    const root = ReactNoop.createRoot();
    function Child({text}) {
      useLayoutEffect(() => {
        Scheduler.unstable_yieldValue('Mount layout');
  ...
  // @gate experimental || www
  it.skipIf(__VUACT_SKIP_TEST2__, 'hides updated nodes inside an already hidden tree', async () => {
    const root = ReactNoop.createRoot();
    await act(async () => {
      root.render(
        <Offscreen mode="hidden">
  ...
  // @gate variant
  it.skipIf(__VUACT_SKIP_TEST2__, 'revealing a hidden tree at high priority does not cause tearing', async () => {
