...
describe('ReactNewContext', () => {
  ...
  function sharedContextTests(label, getConsumer) {
    describe(`reading context with ${label}`, () => {
      ...
      // @gate www
      it.skipIf(__VUACT_SKIP_TEST__, "context consumer doesn't bail out inside hidden subtree", () => {
  ...
  describe('Context.Provider', () => {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'warns if multiple renderers concurrently render the same context', () => {
    ...
    it('provider does not bail out if legacy context changed above', () => {
      ...
      // XXX vuact legacy context
      expect(Scheduler).toFlushAndYield(!__VUACT_SKIP_TEST__ ? ['LegacyProvider', 'App', 'Child'] : ['LegacyProvider']);
      expect(ReactNoop.getChildren()).toEqual([span('Child')]);
  ...
  describe('Context.Consumer', () => {
    it.skipIf(__VUACT_SKIP_TEST__, 'warns if child is not a function', () => {
      spyOnDev(console, 'error');
      const Context = React.createContext(0);
  ...
  describe('readContext', () => {
    ...
    // once that exists.
    // @gate FIXME
    it.skipIf(__VUACT_SKIP_TEST__, 'can read the same context multiple times in the same function', () => {
  ...
  describe('useContext', () => {
    it.skipIf(__VUACT_SKIP_TEST__, 'throws when used in a class component', () => {
      const Context = React.createContext(0);
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'unwinds after errors in complete phase', () => {
    const Context = React.createContext(0);
  ...
  describe.skipIf(__VUACT_SKIP_TEST__, 'fuzz test', () => {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'hard-coded tests', () => {
      const {simulate} = ContextSimulator(5);
      simulate('randomSeed', [flush(3), update('A', 4)]);
    });

    it.skipIf(__VUACT_SKIP_TEST__, 'generated tests', () => {
      const {simulate} = ContextSimulator(5);
