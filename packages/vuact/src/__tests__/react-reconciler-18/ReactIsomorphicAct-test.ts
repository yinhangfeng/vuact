...
describe('isomorphic act()', () => {
  ...
  // @gate __DEV__
  test.skipIf(__VUACT_SKIP_TEST__, 'bypasses queueMicrotask', async () => {
  ...
  // @gate __DEV__
  test.skipIf(__VUACT_SKIP_TEST__, 'in legacy mode, updates are batched', () => {
  ...
  // @gate __DEV__
  test.skipIf(__VUACT_SKIP_TEST__, 'in legacy mode, in an async scope, updates are batched until the first `await`', async () => {
    const root = ReactNoop.createLegacyRoot();
