...
describe('memo', () => {
  ...
  function sharedTests(label, memo) {
    describe(`${label}`, () => {
      ...
      it.skipIf(__VUACT_SKIP_TEST__, "does not bail out if there's a context change", async () => {
      ...
      it.skipIf(__VUACT_SKIP_TEST__, 'does not drop lower priority state updates when bailing out at higher pri (simple)', async () => {
      ...
      it.skipIf(__VUACT_SKIP_TEST__, 'does not drop lower priority state updates when bailing out at higher pri (complex)', async () => {
        const { useState } = React;
