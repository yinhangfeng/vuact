...
describe('ReactTypeScriptClass', function() {
  ...
  it('setState through an event handler', async function() {
    test(
      React.createElement(BoundEventHandler, {initialValue: 'foo'}),
      'DIV',
      'foo'
    );
    await act(() => attachedListener());
  ...
  it('renders using forceUpdate even when there is no state', async function() {
    test(
  ...
  it('will call all the normal life cycle methods', async function() {
    lifeCycles = [];
    ...
    lifeCycles = []; // reset
    await act(() => root.unmount(container));
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should throw AND warn when trying to access classic APIs', function() {
    const ref = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'supports string refs', function() {
    const ref = React.createRef();
    expect(() => {
