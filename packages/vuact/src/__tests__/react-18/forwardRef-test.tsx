...
describe('forwardRef', () => {
  ...
  it('should update refs when switching between children', async () => {
    ...
    await ReactNoop.render(<RefForwardingComponent ref={ref} setRefOnDiv={true} />);
    expect(Scheduler).toFlushWithoutYielding();
    // expect(ref.current.type).toBe('div');
    expect(ref.current.tagName).toBe('DIV');

    await ReactNoop.render(<RefForwardingComponent ref={ref} setRefOnDiv={false} />);
    expect(Scheduler).toFlushWithoutYielding();
  ...
  it('should support propTypes and defaultProps', () => {
    ...
    // ]);
    expect(ref.current.innerHTML).toEqual('foobar');
    ...
    //   {text: 'foo', hidden: false},
    // ]);
    expect(ref.current.innerHTML).toEqual('defaultfoo');
