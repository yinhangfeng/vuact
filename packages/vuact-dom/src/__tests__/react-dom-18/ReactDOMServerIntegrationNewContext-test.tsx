...
describe.skip('ReactDOMServerIntegration', () => {
  beforeEach(() => {
    resetModules();
  });

  describe.skipIf(__VUACT_SKIP_TEST__, 'context', function() {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'does not pollute parallel node streams', () => {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'does not pollute parallel node streams when many are used', () => {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'does not pollute sync renders after an error', async () => {
      ...
      expect(async () => {
        await ReactDOMServer.renderToString(AppWithUser('Casper'));
      }).toThrow('Boo');
