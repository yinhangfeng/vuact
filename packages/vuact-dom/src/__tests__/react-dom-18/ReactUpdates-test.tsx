...
describe('ReactUpdates', () => {
  ...
  it('should batch state and props together', () => {
    ...
    ReactDOM.unstable_batchedUpdates(function () {
      // TODO vuact3 render 是同步的不受 batchedUpdates 影响
  ...
  it('should flush updates in the correct order', async () => {
    ...
    // TODO vuact3 react componentDidUpdate setState callback 中的 setState 会被 batch，且 child.setState() parnet.setState 会按 depth 顺序更新
    // vue child.setState() parent.setState 会按 setState 顺序更新
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'throws in setState if the update callback is not a function', () => {
    function Foo() {
      this.a = 1;
      this.b = 2;
    }
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'throws in forceUpdate if the update callback is not a function', () => {
    function Foo() {
      this.a = 1;
      this.b = 2;
    }
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'handles reentrant mounting in synchronous mode', () => {
    let mounts = 0;
      ...
      ReactDOM.render(
        <Editor
          onChange={(newProps) => {
            ...
            // TODO vuact2 外部 render 未结束的时候进行新的 render 会触发 vue BUG
            render();
          }}
          {...props}
        />,
  ...
  // @gate www
  it.skipIf(__VUACT_SKIP_TEST__, 'delays sync updates inside hidden subtrees in Concurrent Mode', () => {
    const container = document.createElement('div');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'resets the update counter for unrelated updates', () => {
    const container = document.createElement('div');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'does not fall into an infinite update loop', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'does not fall into an infinite update loop with useLayoutEffect', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'can recover after falling into an infinite update loop', () => {
    class NonTerminating extends React.Component {
      state = { step: 0 };
      componentDidMount() {
        this.setState({ step: 1 });
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'does not fall into mutually recursive infinite update loop with same container', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'does not fall into an infinite error loop', () => {
    function BadRender() {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'prevents infinite update loop triggered by synchronous updates in useEffect', () => {
