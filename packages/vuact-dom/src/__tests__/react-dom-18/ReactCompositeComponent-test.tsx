...
describe('ReactCompositeComponent', () => {
  ...
  if (!gate(flags => flags.disableModulePatternComponents)) {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'should support module pattern components', () => {
      function Child({test}) {
        return {
          render() {
            return <div>{test}</div>;
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should warn when rendering a class with a render method that does not extend React.Component', () => {
    const container = document.createElement('div');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should warn about `setState` in render', () => {
    const container = document.createElement('div');
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should cleanup even if render() fatals', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should trigger componentWillReceiveProps for context changes', async () => {
    ...
    // TODO vuact3 没有 contextTypes 的组件不会因为 context 变化而更新
  ...
  it('should support objects with prototypes as state', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'prepares new child before unmounting old', () => {
    const log = [];
    ...
    expect(log).toEqual([
      ...
      'A render',
      'A componentDidMount',
      // TODO vuact2 会在这里调用 A componentWillUnmount
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should return a meaningful warning when constructor is returned', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should return error if render is not defined', () => {
