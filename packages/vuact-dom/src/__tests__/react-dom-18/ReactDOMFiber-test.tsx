...
describe('ReactDOMFiber', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'finds the first child when a component returns a fragment', () => {
    ...
    // TODO vuact2 vue 对于 render 返回数组会包裹 Fragment，而 Fragment 渲染会在前后插入空字符串 (processFragment fragmentStartAnchor fragmentEndAnchor)
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'finds the first child even when fragment is nested', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'finds the first child even when first child renders null', () => {
    ...
    // TODO vuact2 vue 可能会增加 Text 与 Comment
    expect(container.childNodes.length).toBe(2);
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'renders an empty fragment', () => {
    const Div = () => <div />;
    const EmptyFragment = () => <></>;
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should keep track of namespace across portals (simple)', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should keep track of namespace across portals (medium)', () => {
    assertNamespacesMatch(
      <svg {...expectSVG}>
        <image {...expectSVG} />
        {usePortal(<div {...expectHTML} />)}
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should keep track of namespace across portals (complex)', () => {
    assertNamespacesMatch(
      <div {...expectHTML}>
        {usePortal(
          <svg {...expectSVG}>
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should unwind namespaces on uncaught errors', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should unwind namespaces on caught errors in a portal', () => {
    function BrokenRender() {
      throw new Error('Hello');
    }
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should not onMouseLeave when staying in the portal', () => {
    const portalContainer = document.createElement('div');
    document.body.appendChild(portalContainer);
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'does not fire mouseEnter twice when relatedTarget is the root node', () => {
    let ops = [];
    let target = null;
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'listens to events that do not exist in the Portal subtree', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should throw on bad createPortal argument', () => {
    expect(() => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should not update event handlers until commit', async () => {
    ...
    const node = ReactDOM.findDONNode(inst);
    ...
    // TODO vuact2 会先 render Example 完成 commit 再 Click constructor
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should render a text component with a text DOM node on the same document as the container', () => {
