...
// @gate __DEV__
it.skipIf(__VUACT_SKIP_TEST__, "should keep flushing effects until they're done", () => {
  function App() {
    const [ctr, setCtr] = React.useState(0);
    React.useEffect(() => {
      if (ctr < 5) {
...
// @gate __DEV__
it.skipIf(__VUACT_SKIP_TEST2__, 'should flush effects only on exiting the outermost act', () => {
  function App() {
    React.useEffect(() => {
      yields.push(0);
    });
...
// @gate __DEV__
it.skipIf(__VUACT_SKIP_TEST2__, 'can handle cascading promises', async () => {
