...
describe('ReactMount', () => {
  ...
  describe('unmountComponentAtNode', () => {
    it.skipIf(__VUACT_SKIP_TEST__, 'throws when given a non-node', () => {
  ...
  it('does not warn if mounting into left padded rendered markup', async () => {
    const container = document.createElement('container');
    container.innerHTML = await ReactDOMServer.renderToString(<div />) + ' ';

    // This should probably ideally warn but we ignore extra markup at the root.
    ReactDOM.hydrate(<div />, container);
  });

  it('should warn if mounting into right padded rendered markup', async () => {
    const container = document.createElement('container');
    container.innerHTML = ' ' + await ReactDOMServer.renderToString(<div />);
  ...
  it('should account for escaping on a checksum mismatch', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should warn if the unmounted node was rendered by another copy of React', () => {
    jest.resetModules();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'initial mount of legacy root is sync inside batchedUpdates, as if it were wrapped in flushSync', () => {
  ...
  describe.skipIf(__VUACT_SKIP_TEST__, 'mount point is a comment node', () => {
    let containerDiv;
    let mountPoint;
