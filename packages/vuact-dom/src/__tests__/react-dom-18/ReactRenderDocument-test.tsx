...
describe('rendering React components at document', () => {
  ...
  describe('with new explicit hydration API', () => {
    it('should be able to adopt server markup', async () => {
      ...
      const markup = await ReactDOMServer.renderToString(<Root hello="world" />);
    ...
    it('should not be able to unmount component from document node', async () => {
      class Root extends React.Component {
        render() {
          return (
            <html>
      ...
      const markup = await ReactDOMServer.renderToString(<Root />);
      const testDocument = getTestDocument(markup);
    ...
    it('should not be able to switch root constructors', async () => {
      ...
      const markup = await ReactDOMServer.renderToString(<Component />);
      const testDocument = getTestDocument(markup);
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'cannot render over an existing text child at the root', () => {
    ...
    it.skipIf(__VUACT_SKIP_TEST__, 'should render w/ no markup to full document', () => {
    ...
    it('supports findDOMNode on full-page components', async () => {
      const tree = (
        <html>
          <head>
            <title>Hello World</title>
      ...
      const markup = await ReactDOMServer.renderToString(tree);
      const testDocument = getTestDocument(markup);
