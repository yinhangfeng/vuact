...
describe('ReactBrowserEventEmitter', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST2__, 'should set currentTarget', () => {
    ...
    // TODO vuact3 有概率会失败，vue render 之后有可能事件 listener 是异步绑定的?
    CHILD.click();
    expect(idCallOrder.length).toBe(3);
    expect(idCallOrder[0]).toBe(CHILD);
    expect(idCallOrder[1]).toBe(PARENT);
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should support stopPropagation()', () => {
    putListener(CHILD, ON_CLICK_KEY, recordID.bind(null, CHILD));
    putListener(
      PARENT,
      ON_CLICK_KEY,
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should support overriding .isPropagationStopped()', () => {
    // Ew. See D4504876.
    putListener(CHILD, ON_CLICK_KEY, recordID.bind(null, CHILD));
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should invoke handlers that were removed while bubbling', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should not invoke newly inserted handlers while bubbling', () => {
    const handleParentClick = jest.fn();
    const handleChildClick = function(event) {
      putListener(PARENT, ON_CLICK_KEY, handleParentClick);
    };
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should have mouse enter simulated by test utils', () => {
