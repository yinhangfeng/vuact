...
describe('ReactDOMServerLifecycles', () => {
  ...
  it('should invoke the correct legacy lifecycle hooks', async () => {
    const log = [];
    ...
    await ReactDOMServer.renderToString(<Outer />);
    expect(log).toEqual([
  ...
  it('should invoke the correct new lifecycle hooks', async () => {
    const log = [];
  ...
  it('should not invoke unsafe cWM if static gDSFP is present', async () => {
  ...
  it('should update instance.state with value returned from getDerivedStateFromProps', async () => {
    ...
    const markup = await ReactDOMServer.renderToString(<Grandparent />);
  ...
  it('should warn if getDerivedStateFromProps returns undefined', async () => {
    ...
    expect(async () => await ReactDOMServer.renderToString(<Component />)).toErrorDev(
      'Component.getDerivedStateFromProps(): A valid state object (or null) must ' +
        'be returned. You have returned undefined.',
    );

    // De-duped
    await ReactDOMServer.renderToString(<Component />);
  });
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should invoke both deprecated and new lifecycles if both are present', async () => {
    const log = [];
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'tracks state updates across components', () => {
    class Outer extends React.Component {
      UNSAFE_componentWillMount() {
        this.setState({x: 1});
      }
  ...
  it('should not invoke cWM if static gDSFP is present', async () => {
    class Component extends React.Component {
      state = {};
      static getDerivedStateFromProps() {
        return null;
  ...
  it('should warn about deprecated lifecycle hooks', async () => {
    ...
    // De-duped
    await ReactDOMServer.renderToString(<MyComponent />);
  });
