...
describe('ReactDOMOption', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should ignore null/undefined/false children without warning', () => {
    const stub = (
      <option>
        {1} {false}
        {true}
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'should throw on object children', () => {
    expect(() => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'generates a warning and hydration error when an invalid nested tag is used as a child', async () => {
    ...
    container.innerHTML = await ReactDOMServer.renderToString(children);
