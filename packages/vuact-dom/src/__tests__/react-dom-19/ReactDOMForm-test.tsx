...
describe('ReactDOMForm', () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'can read the clicked button in the formdata event', async () => {
    const inputRef = React.createRef();
    const buttonRef = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'allows a non-react html formaction to be invoked', async () => {
    let actionCalled = false;
  ...
  it('form actions are transitions', async () => {
    ...
    if (!__VUACT_SKIP_TEST__) {
      // TODO vuact1 react startTransition 触发了 suspense，则在 suspense 结束之前 transitiojn 状态会持续
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'multiple form actions', async () => {
    const formRef = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'form actions can be asynchronous', async () => {
    const formRef = React.createRef();
  ...
  it(
    ...
    async () => {
      ...
      await act(() => startTransition(() => action('Third action')));
    },
  );
  ...
  // TODO vuact Suspense
  it.skipIf(__VUACT_SKIP_TEST__, 'useActionState does not wrap action in a transition unless dispatch is in a transition', async () => {
    let dispatch;
    function App() {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'uncontrolled form inputs are reset after the action completes', async () => {
    const formRef = React.createRef();
    const inputRef = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'requestFormReset schedules a form reset after transition completes', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'reset multiple forms in the same transition', async () => {
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'requestFormReset throws if the form is not managed by React', async () => {
    container.innerHTML = `
      <form id="myform">
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'requestFormReset throws on a non-form DOM element', async () => {
    const root = ReactDOMClient.createRoot(container);
    const ref = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'warns if requestFormReset is called outside of a transition', async () => {
    const formRef = React.createRef();
    const inputRef = React.createRef();
  ...
  it.skipIf(__VUACT_SKIP_TEST__, "regression: submitter's formAction prop is coerced correctly before checking if it exists", async () => {
  ...
  it('useFormStatus is activated if startTransition is called ' +
  ...
  it.skipIf(__VUACT_SKIP_TEST__, 'useFormStatus is not activated if event is not preventDefault-ed', async () => {
    function Output({value}) {
      const {pending} = useFormStatus();
      return <Text text={pending ? `${value} (pending...)` : value} />;
    }
