...
const MySuspense = defineComponent({
  setup: () => {
    const suspended = ref(false);
    const update = ref(0);

    onErrorCaptured((err) => {
      console.log('MySuspense onErrorCaptured', err);
    ...
    const MyKeepAlive = defineComponent({
      setup(props, { slots }) {
        return () => {
          return h(KeepAlive, {}, suspended.value ? undefined : slots);
        };
      },
    });
    ...
    return () => {
      ...
      console.log('MySuspense render');

      return [
        h('div', {}, 'MySuspense'),
        // h(Comp1, {}),
        // h(KeepAlive, {}, !suspended.value && h(CompParent, {})),
        h(MyKeepAlive, {}, h(CompParent, {})),
        // suspended.value && h('div', {}, 'suspended'),
        h(
          'button',
          {
            onClick: () => {
...
<template>
  ...
  <div>
    <!-- <KeepAlive>
      <Comp1 v-if="showComp1" :key="key" />
      <Comp2 v-else :key="key" />
    </KeepAlive> -->
    <MySuspense />
  </div>
</template>
